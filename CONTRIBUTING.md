...
### TLDR
```
docker pull ghcr.io/opencyphal/toolshed:ts22.4.6
git clone {this repo}
cd CETL
docker run --rm -it -v ${PWD}:/repo ghcr.io/opencyphal/toolshed:ts22.4.6
...
```
docker run --rm -v ${PWD}:/repo ghcr.io/opencyphal/toolshed:ts22.4.6 ./build-tools/bin/verify.py build-danger-danger-cetlvast-clang-format-in-place
```
